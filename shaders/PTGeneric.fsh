...
void main() {
...
#ifdef PT_FEATURE_TEXTURE
#ifdef PT_FEATURE_UV1_SCALE
    color = texture2D(CC_Texture0, texCoord / texCoord1 / textureScale + textureOffset) * color;
#else // PT_FEATURE_UV1_SCALE
    color = texture2D(CC_Texture0, texCoord / textureScale + textureOffset) * color;
#endif // PT_FEATURE_UV1_SCALE
    ...
    if (viewDepth > fogStartDistance) {
        ...
        gl_FragColor = mix(gl_FragColor, vec4(fogColor.r, fogColor.g, fogColor.b, a), clamp(a, 0.0, 1.0));
    }
#endif // PT_FEATURE_FOG



#ifndef PT_FEATURE_OPACITY
    gl_FragColor.a = 1.0;
